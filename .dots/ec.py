...
def main(stdscr):
    ...
    def print_menu():
        if lang=='tr':
            stdscr.addstr(0,0,"Şifre Oluşturucu",curses.A_REVERSE if selected_row==0 else curses.A_NORMAL)
            stdscr.addstr(1,0,"Yön tuşlarıyla gezin, Enter ile değiştir.",curses.A_REVERSE if selected_row==1 else curses.A_NORMAL)
            stdscr.addstr(2,0,"'q' ile çık.",curses.A_REVERSE if selected_row==2 else curses.A_NORMAL)
            ...
            stdscr.addstr(11,0,"Şifre: ",curses.A_REVERSE if selected_row==9 else curses.A_NORMAL);stdscr.addstr(12,0,p if p else "Şifre oluşturulmadı.",curses.A_NORMAL)
        else:
            stdscr.addstr(0,0,"Password Generator",curses.A_REVERSE if selected_row==0 else curses.A_NORMAL)
            stdscr.addstr(1,0,"Use arrows, Enter to change.",curses.A_REVERSE if selected_row==1 else curses.A_NORMAL)
            stdscr.addstr(2,0,"'q' to quit.",curses.A_REVERSE if selected_row==2 else curses.A_NORMAL)
            ...
            stdscr.addstr(11,0,"Password: ",curses.A_REVERSE if selected_row==9 else curses.A_NORMAL);stdscr.addstr(12,0,p if p else "No password generated.",curses.A_NORMAL)
        stdscr.addstr(14,0,"'g' ile şifre oluştur.",curses.A_NORMAL)
    print_menu()
    while True:
        k=stdscr.getch()
        if k==curses.KEY_UP: selected_row=(selected_row-1)%10
        elif k==curses.KEY_DOWN: selected_row=(selected_row+1)%10
        elif k==ord('1')and selected_row==3:
            ...
            curses.noecho()
        elif k==ord('2')and selected_row==4:u=not u
        elif k==ord('3')and selected_row==5:lc=not lc
        elif k==ord('4')and selected_row==6:n=not n
        elif k==ord('5')and selected_row==7:
            s=not s
            if s:
                ...
                curses.noecho()
        elif k==ord('6')and selected_row==8:e=not e
        elif k==ord('g'):
