...
def main(stdscr):
    ...
    stdscr.refresh()
    lang_key = stdscr.getch()
    lang = 'tr' if lang_key == ord('T') or lang_key == ord('t') else 'en'
    curses.curs_set(0)
    stdscr.clear()
    ...
    while True:
        ...
        if lang == 'tr':
            ...
            stdscr.addstr(4, 0, f"1. Şifre Uzunluğu: {l} (Değiştirmek için Enter)")
            ...
            stdscr.addstr(12, 0, f"{p}" if p else "Henüz şifre oluşturulmadı.")
            ...
            stdscr.addstr(4, 0, f"1. Password Length: {l} (Press Enter to change)")
            ...
            stdscr.addstr(12, 0, f"{p}" if p else "No password generated yet.")
        ...
        if k == ord('1'):
            stdscr.clear()
            stdscr.addstr(0, 0, "Şifre uzunluğunu girin: ")
            stdscr.refresh()
            curses.echo()
            length_input = stdscr.getstr(0, 24).decode('utf-8')
            if length_input.isdigit():
                l = int(length_input)
            else:
                stdscr.addstr(1, 0, "Geçersiz giriş, lütfen sayı girin.")
                stdscr.refresh()
                curses.napms(1000)
            curses.noecho()
            ...
            if s:
                stdscr.clear()
                stdscr.addstr(0, 0, "Kullanılacak sembolleri girin: ")
                stdscr.refresh()
                curses.echo()
            ...
            if not (u or lc or n or s):
                stdscr.addstr(1, 0, "Hata: En az bir karakter türü seçilmelidir.")
                stdscr.refresh()
                curses.napms(1000)
            else:
