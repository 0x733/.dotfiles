...
def generate_password(length, u, l, n, s, e, symbols):
    ...
    return p
def main(stdscr):
    stdscr.addstr(0, 0, "Lütfen dil seçin: (E) English, (T) Türkçe")
    stdscr.refresh()
    ...
    while True:
        ...
        if lang == 'tr':
            ...
            stdscr.addstr(1, 0, "Yön tuşlarını kullanın, seçenekleri değiştirmek için Enter'a basın.")
            stdscr.addstr(2, 0, "Çıkmak için 'q' tuşuna basın.")
            stdscr.addstr(4, 0, f"1. Şifre Uzunluğu: {l}")
            ...
            stdscr.addstr(8, 0, f"5. Semboller: {'Evet' if s else 'Hayır'}, {symbols}")
            stdscr.addstr(9, 0, f"6. Tekrarlayan Karakterler Hariç: {'Evet' if e else 'Hayır'}")
            stdscr.addstr(11, 0, "Oluşturulan Şifre: ")
            stdscr.addstr(12, 0, p if p else "Henüz şifre oluşturulmadı.")
        else:
            stdscr.addstr(0, 0, "Advanced Password Generator (Terminal)")
            stdscr.addstr(1, 0, "Use arrow keys to navigate, press Enter to change options.")
            stdscr.addstr(2, 0, "Press 'q' to exit.")
            stdscr.addstr(4, 0, f"1. Password Length: {l}")
            ...
            stdscr.addstr(8, 0, f"5. Include Symbols: {'Yes' if s else 'No'}, {symbols}")
            stdscr.addstr(9, 0, f"6. Exclude Duplicate Characters: {'Yes' if e else 'No'}")
            stdscr.addstr(11, 0, "Generated Password: ")
            stdscr.addstr(12, 0, p if p else "No password generated yet.")
        stdscr.addstr(14, 0, "Press 'g' to generate a new password.")
        k = stdscr.getch()
        if k == ord('1'):
            stdscr.clear(); stdscr.addstr(0, 0, "Şifre uzunluğunu girin (Maksimum 128): "); stdscr.refresh(); curses.echo()
            length_input = stdscr.getstr(0, 38).decode('utf-8')
            ...
            s = not s
            if s:
                stdscr.clear(); stdscr.addstr(0, 0, "Kullanılacak sembolleri girin: "); stdscr.refresh(); curses.echo()
                symbols_input = stdscr.getstr(0, 32).decode('utf-8')
                symbols = symbols_input if symbols_input else string.punctuation
                curses.noecho()
        elif k == ord('6'): e = not e
        elif k == ord('g'):
            if not (u or lc or n or s): stdscr.addstr(1, 0, "En az bir karakter türü seçmelisiniz."); stdscr.refresh(); curses.napms(1000)
