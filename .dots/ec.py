...
def generate_password(length, u, l, n, s, e):
    c = (string.ascii_uppercase if u else '') + (string.ascii_lowercase if l else '') + (string.digits if n else '') + (string.punctuation if s else '')
    p, used = "", set()
    while len(p) < length:
        ch = random.choice(c)
...
def main(stdscr):
    ...
    l, u, lc, n, s, e, p = 12, True, True, True, True, True, ""
    while 1:
        stdscr.clear()
        if lang == 'tr':
            ...
            stdscr.addstr(8, 0, f"5. Semboller (@-$): {'Evet' if s else 'Hayır'}")
            ...
            stdscr.addstr(8, 0, f"5. Include Symbols (@-$): {'Yes' if s else 'No'}")
        ...
        elif k == ord('5'): s = not s
        elif k == ord('6'): e = not e
        elif k == ord('g'): p = generate_password(l, u, lc, n, s, e)
        elif k == ord('q'): break
        stdscr.refresh()
curses.wrapper(main)